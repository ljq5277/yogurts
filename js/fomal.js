...
function showWelcome() {
  ...
  let dist = getDistance(108, 34, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime() {
  ...
  var start = new Date("04/08/2023 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("04/08/2023 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(/assets/background.png);
    --darkmode-bg:url(/assets/background.png);
    --mobileday-bg: url(/assets/mobilebackground.png);
    --mobilenight-bg: url(/assets/mobilebackground.png);
  }`;
}
// 切换背景主函数
function changeBg(s) {
